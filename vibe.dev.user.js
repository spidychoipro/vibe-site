...
(function () {
  ...
  const CSS = `
    #__vibe_root, #__vibe_root * { box-sizing: border-box; }
    #__vibe_root {
      position: fixed;
      inset: 0;
      z-index: 9999;
    ...
    #__vibe_root .line { white-space: pre-wrap; word-break: break-word; min-height: 1.65em; }
    #__vibe_root .cur {
    ...
    #__vibe_root .prompt { color: #22cc55; }
    #__vibe_root .dim { color: #1d8f40; }
    #__vibe_root .title {
      font-size: 30px;
      font-weight: bold;
      color: #b6ffc9;
    ...
    #__vibe_root .manifesto { margin: 18px 0 0 8px; }
    #__vibe_root .manifesto .num { color: #1d8f40; }
    #__vibe_root .btn {
      display: inline-block;
      margin-top: 30px;
      padding: 10px 18px;
    ...
    #__vibe_root .btn:hover { background: rgba(51,255,102,0.15); }
    #__vibe_root .easter {
      margin-top: 14px;
      color: #ff66aa;
      min-height: 1.65em;
    }
    #__vibe_root a { color: #33ff66; text-decoration: underline; }
    #__vibe_root .fade { opacity: 0; transition: opacity 0.8s ease; }
    #__vibe_root .fade.show { opacity: 1; }
    #__vibe_root footer { margin-top: 40px; color: #1d8f40; font-size: 13px; }
  `;
  ...
  function injectStyle() {
    if (document.getElementById('__vibe_style')) return;
  ...
  function buildManifesto() {
    ...
    footer.textContent = 'vibe-site v0.3.0 · ';
    const link = document.createElement('a');
    link.href = 'https://spidychoipro.github.io/vibe-site/vibe.user.js';
    link.target = '_blank';
    link.rel = 'noopener noreferrer';
    link.textContent = 'vibe.user.js';
  ...
  function isTarget() {
    const hostOk = HOSTS.some(
      (h) => location.origin === h.origin && location.pathname === h.pathname
    );
  ...
  function start() {
    if (window.__VIBE_SITE_RUNNING__) return;
    if (!isTarget()) return;
    window.__VIBE_SITE_RUNNING__ = true;
    injectStyle();
    buildRoot();
    appendLine('');
    appendLine('vibe-site/~ root shell (c) 2026', 'dim');
    appendLine('');
